...
<body>
    ...
    <div class="dropdown">
        <span> <li> <li style="margin-top:-10px;"><a href="grade11.php">Grade 11</a></li></span>
        <div class="dropdown-content">
            <a href="#mathematics">Mathematics</a>
            <a href="#biology">Biology</a>
            <a href="#physics">Physics</a>
    ...
    <h1 style="text-align: center;">GRADE 9 MATERALS</h1><hr>
...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadMaths'])) {
    ...
    if ($fileType == "pdf") {
        ...
        if (move_uploaded_file($file["tmp_name"], $targetFile)) {
            // Insert file information into the maths_files database table
            $stmt = $conn->prepare("INSERT INTO maths_files (file_name, file_path) VALUES (?, ?)");
...
<div class="gallery-item">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadCiv'])) {
        ...
        if ($fileType == "pdf") {
            // Move the uploaded file to the server's upload directory
            if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                // Prepare the SQL statement to insert file information into the bio9_files table
                $stmt = $conn->prepare("INSERT INTO bio9_files (file_name, file_path) VALUES (?, ?)");
                $stmt->bind_param("ss", $file["name"], $targetFile);
                if ($stmt->execute()) {
    ...
    <div class="gallery-item">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUpload'])) {
            ...
            if ($fileType == "pdf") {
                ...
                if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                    ...
                    $stmt = $conn->prepare("INSERT INTO chem_files (file_name, file_path) VALUES (?, ?)");
                    $stmt->bind_param("ss", $file["name"], $targetFile);
                    if ($stmt->execute()) {
        ...
        <body>
            <h2>Upload PDF</h2>
            <form method="POST" enctype="multipart/form-data">
                <input type="file" name="pdfUpload" accept="application/pdf" required>
                <button type="submit">Upload</button>
            </form>
            <?php
            if ($result->num_rows > 0) {
                ...
                while ($row = $result->fetch_assoc()) {
                    $fileName = htmlspecialchars($row['file_name']);
                    $filePath = htmlspecialchars($row['file_path']);
                    $fileId = $row['id']; // Assuming there's an ID field in the table
                    echo "<li>";
                    echo "<a href='$filePath' target='_blank'>$fileName</a>";
                    echo " <a href='$filePath' download><button>Download</button></a>";
                }
...
<div class="gallery-item">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadPhy'])) {
        ...
        if ($fileType == "pdf") {
            ...
            if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                // Insert file details into the phy9_files table
                $stmt = $conn->prepare("INSERT INTO phy9_files (file_name, file_path) VALUES (?, ?)");
                $stmt->bind_param("ss", $file["name"], $targetFile);
                if ($stmt->execute()) {
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadHisto'])) {
        ...
        if ($fileType == "pdf") {
            // Move the uploaded file to the upload directory
            if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                // Insert file details into the histo9_files table
                $stmt = $conn->prepare("INSERT INTO histo9_files (file_name, file_path) VALUES (?, ?)");
                $stmt->bind_param("ss", $file["name"], $targetFile);
                if ($stmt->execute()) {
    ...
    $query = "SELECT * FROM histo9_files"; 
    $result = $conn->query($query);
    ?>
...
<div class="gallery-item">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadGeo'])) {
        ...
        if ($fileType == "pdf") {
            // Move the uploaded file to the server's upload directory
            if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                // Insert file information into the geo9_files table
                $stmt = $conn->prepare("INSERT INTO geo9_files (file_name, file_path) VALUES (?, ?)");
                $stmt->bind_param("ss", $file["name"], $targetFile);
                if ($stmt->execute()) {
    ...
    // Display uploaded Geography files
    $query = "SELECT * FROM geo9_files"; 
    $result = $conn->query($query);
    ?>
...
<div class="gallery-item">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadCiv'])) {
        ...
        if ($fileType == "pdf") {
            // Move the uploaded file to the server's upload directory
            if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                // Insert file information into the civ9_files table
                $stmt = $conn->prepare("INSERT INTO civ9_files (file_name, file_path) VALUES (?, ?)");
                $stmt->bind_param("ss", $file["name"], $targetFile);
                if ($stmt->execute()) {
...
<div class="gallery-item">
    ...
    if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadEco'])) {
        ...
        if ($fileType == "pdf") {
            // Move the uploaded file to the server's upload directory
            if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                // Insert file information into the eco9_files table
                $stmt = $conn->prepare("INSERT INTO eco9_files (file_name, file_path) VALUES (?, ?)");
                $stmt->bind_param("ss", $file["name"], $targetFile);
                if ($stmt->execute()) {
