...
<body>
    ...
    <h1 style="text-align: center;">GRADE 12 MATERALS</h1><hr>
    <div class="gallery-containered">
    <div class="gallery">
        
        <!-- Mathematics PDF Section -->
        <div class="gallery-item">
            <div id="mathematics">
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadMaths'])) {
                    ...
                    if ($fileType == "pdf") {
                        ...
                        if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                            // Insert file information into the maths12_files database table
                            $stmt = $conn->prepare("INSERT INTO maths12_files (file_name, file_path) VALUES (?, ?)");
                ...
                $query = "SELECT * FROM maths12_files"; 
        ...
        <div class="gallery-item">
            <div id="biology">
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadBio'])) {
                    ...
                    if ($fileType == "pdf") {
                        if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                            // Insert file information into the bio12_files database table
                            $stmt = $conn->prepare("INSERT INTO bio12_files (file_name, file_path) VALUES (?, ?)");
                            $stmt->bind_param("ss", $file["name"], $targetFile);
                            if ($stmt->execute()) {
        ...
        <div class="gallery-item">
            <div id="chemistry">
                ...
                if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadChem'])) {
                    ...
                    if ($fileType == "pdf") {
                        if (move_uploaded_file($file["tmp_name"], $targetFile)) {
                            // Insert file information into the chem12_files database table
                            $stmt = $conn->prepare("INSERT INTO chem12_files (file_name, file_path) VALUES (?, ?)");
                            $stmt->bind_param("ss", $file["name"], $targetFile);
                            if ($stmt->execute()) {
                                echo "Chemistry PDF uploaded and inserted successfully!";
                            } else {
                                echo "Error: " . $stmt->error;
                            }
                            $stmt->close();
                ...
                $query = "SELECT * FROM chem12_files"; 
                $result = $conn->query($query);
                ?>
    ...
    <div id="physics">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadPhy'])) {
            ...
            $targetFile = $targetDir . basename($file["name"]);
            $fileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));  // Validate file type

            if ($fileType == "pdf") {  // Allow only PDF files
                if (move_uploaded_file($file["tmp_name"], $targetFile)) {  // Move file to server
                    // Insert file details into 'phy12_files' table
                    $stmt = $conn->prepare("INSERT INTO phy12_files (file_name, file_path) VALUES (?, ?)");
                    $stmt->bind_param("ss", $file["name"], $targetFile);
                    if ($stmt->execute()) {
        ...
        // Fetch uploaded Physics files from database
        $query = "SELECT * FROM phy12_files";
        $result = $conn->query($query);
        ?>
        ...
        <?php
        // Display the uploaded files as a list
        if ($result->num_rows > 0) {
            echo "<ul>";
            while ($row = $result->fetch_assoc()) {
                $fileName = htmlspecialchars($row['file_name']);
...
<div class="gallery-item">
    <div id="history">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadHisto'])) {
            ...
            if ($fileType == "pdf") {  // Allow only PDF files
                if (move_uploaded_file($file["tmp_name"], $targetFile)) {  // Move file to server
                    // Insert file details into 'histo12_files' table
                    $stmt = $conn->prepare("INSERT INTO histo12_files (file_name, file_path) VALUES (?, ?)");
                    $stmt->bind_param("ss", $file["name"], $targetFile);
                    if ($stmt->execute()) {
        ...
        // Fetch uploaded History files from database
        $query = "SELECT * FROM histo12_files";
        $result = $conn->query($query);
        ?>
...
<div class="gallery-item">
    <div id="geography">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadGeo'])) {
            ...
            if ($fileType == "pdf") {  // Allow only PDF files
                if (move_uploaded_file($file["tmp_name"], $targetFile)) {  // Move file to server
                    // Insert file details into 'geo12_files' table
                    $stmt = $conn->prepare("INSERT INTO geo12_files (file_name, file_path) VALUES (?, ?)");
                    $stmt->bind_param("ss", $file["name"], $targetFile);
                    if ($stmt->execute()) {
        ...
        // Fetch uploaded Geography files from database
        $query = "SELECT * FROM geo12_files";
        $result = $conn->query($query);
        ?>
...
<div class="gallery-item">
    <div id="civics">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadCiv'])) {
            ...
            if ($fileType == "pdf") {  // Allow only PDF files
                if (move_uploaded_file($file["tmp_name"], $targetFile)) {  // Move file to server
                    // Insert file details into 'civ12_files' table
                    $stmt = $conn->prepare("INSERT INTO civ12_files (file_name, file_path) VALUES (?, ?)");
                    $stmt->bind_param("ss", $file["name"], $targetFile);
                    if ($stmt->execute()) {
        ...
        // Fetch uploaded Civics files from the database
        $query = "SELECT * FROM civ12_files";
        $result = $conn->query($query);
        ?>
...
<div class="gallery-item">
    <div id="economics">
        ...
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadEco'])) {
            ...
            if ($fileType == "pdf") {  // Allow only PDF files
                if (move_uploaded_file($file["tmp_name"], $targetFile)) {  // Move file to server
                    // Insert file details into 'eco12_files' table
                    $stmt = $conn->prepare("INSERT INTO eco12_files (file_name, file_path) VALUES (?, ?)");
                    $stmt->bind_param("ss", $file["name"], $targetFile);
                    if ($stmt->execute()) {
        ...
        // Fetch uploaded Economics files from the database
        $query = "SELECT * FROM eco12_files";
        $result = $conn->query($query);
        ?>
...
<div class="gallery-item">
    <div id="university_entrance_exam">
        <h1>University Entrance Exam PDF</h1>
        <?php
        @include 'config.php';  // Database connection

        // Handle University Entrance Exam file upload
        if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_FILES['pdfUploadUni'])) {
            $file = $_FILES['pdfUploadUni'];
            $targetDir = "uploads/";  // Directory to store uploaded files
            $targetFile = $targetDir . basename($file["name"]);
            $fileType = strtolower(pathinfo($targetFile, PATHINFO_EXTENSION));  // Validate file type
            ...
            if ($fileType == "pdf") {  // Allow only PDF files
                if (move_uploaded_file($file["tmp_name"], $targetFile)) {  // Move file to server
                    // Insert file details into 'university_files' table
                    $stmt = $conn->prepare("INSERT INTO university_files (file_name, file_path) VALUES (?, ?)");
                    $stmt->bind_param("ss", $file["name"], $targetFile);
                    if ($stmt->execute()) {
                        echo "University Entrance Exam file uploaded and inserted successfully!";
                    } else {
                        echo "Error: " . $stmt->error;
                    }
                    $stmt->close();
                ...
                echo "Only PDF files are allowed for University Entrance Exam.";
            }
        }

        // Fetch uploaded University Entrance Exam files from the database
        $query = "SELECT * FROM university_files";
        $result = $conn->query($query);
        ?>

        <h2>Upload University Entrance Exam PDF</h2>
        <form method="POST" enctype="multipart/form-data">
            <input type="file" name="pdfUploadUni" accept="application/pdf" required>
            <button type="submit">Upload</button>
        </form>

        <?php
        // Display uploaded University Entrance Exam files
        if ($result->num_rows > 0) {
            echo "<ul>";
            while ($row = $result->fetch_assoc()) {
                $fileName = htmlspecialchars($row['file_name']);
            ...
            echo "No University Entrance Exam PDF files found.";
        }
